...
"# Bird:股票先知"
"先知会预言某一日股票的最高价和最低价，您可以在页面上随意更改参数或更换算法模型"
"先知比较注重中频量化，低频量化效果不如中频～"
"[repo]()   ||   [blog]()"
"PS：若无特殊需求，请将所有量化因子调至中等水平"
...
# get interval
interval = st.sidebar.selectbox("数据间隔", ['1d', '1wk', '1mo', '3mo'])

#get period
period = st.sidebar.selectbox("学习时期",['1mo','3mo','6mo','1y','2y','5y','10y','max'],index = 2)
...
if len(data)==0:
    ...
    if period == '1mo' or period == '3mo':
        ...
        if interval == '1d' or interval == '1wk':
            horizon = st.sidebar.slider("预测时长", 1, 30, 5)
        else:
            horizon = st.sidebar.slider("预测时长", 1, 15, 5)

    model = st.selectbox('算法模型',['Simple Exponential Smoothing','Halt Model','Holt-Winter Model','Auto Regressive Model',
